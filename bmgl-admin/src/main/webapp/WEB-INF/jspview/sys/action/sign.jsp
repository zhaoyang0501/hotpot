...
<body>
					...
					<div class="ibox-content">
						<form role="form" class="form-inline">
							<div class="form-group">
								<label for="exampleInputEmail2" class="sr-only">活动名称</label> <input
    ...
    function fun_delete(id){
    	layer.confirm('确定删除当前活动？', {
    		  btn: ['确定','取消'] //按钮
    		}, function(){
    			$.ajax({
    		 		   url:  $.common.getContextPath() + "/sys/action/delete?id="+id,
    		 		   success: function(msg){
    		 		     if(msg.code==1){
    		 		    	 toastr.success('操作成功');
    		 		    	 table.draw();
    ...
    function fun_sign(id){
    	layer.confirm('确定报名当前活动？', {
    		  btn: ['确定','取消'] //按钮
    		}, function(){
    			$.ajax({
    		 		   url:  $.common.getContextPath() + "/sys/action/dosign?id="+id,
    		 		   success: function(msg){
    		 		     if(msg.code==1){
    		 		    	 toastr.success('操作成功');
    		 		    	 table.draw();
    ...
    $(document).ready(function(){
    	...
    	////仓库编码	仓库名称	存货编码	规格型号	批号	现存数量	到货在检数量	调拨在途数量	预计入库数量合计	待发货数量	调拨待发数量	可用数量
        	...
        	table=$('#dt_table_view').DataTable( {
        		...
        		"deferLoading": 57,
	            "ajax": {
	                "url":  $.common.getContextPath() + "/sys/action/list",
	                "type": "POST",
	                "async":false,
	                "dataSrc": "datas"
	              },
				...
				"columns" : [{
				 ...
				 "columnDefs": [
				                ...
				                {
				                    "render": function ( data, type, row ) {
				                        return "<a tager='_blank' href='javascript:void(0)' onclick='fun_sign("+data+")'>立即报名 </a>";
				                    },
				                    "targets":7
				                }
